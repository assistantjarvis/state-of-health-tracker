import {
  ...
  SET_DAILY_MEALS_HAS_SYNCED_FALSE,
  SET_MEAL_ENTRIES_SYNCED
...
function addMealToDailyEntry(
  state: DailyMealEntriesState,
  action: Action<{
    entryDate: string
    mealId: string
  }>
): DailyMealEntriesState {
...
export function dailyMealEntriesReducer(
  state = DAILY_MEAL_ENTRIES_INITIAL_STATE,
  action: Action<any>
): DailyMealEntriesState {
